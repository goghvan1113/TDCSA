...
class CustomAttention(nn.Module):
    def __init__(self, hidden_size, sentiment_dict):
        super(CustomAttention, self).__init__()
        self.hidden_size = hidden_size
        self.sentiment_dict = sentiment_dict
        self.sentiment_words = set(sentiment_dict['positive'] + sentiment_dict['negative'])
        self.query = nn.Linear(hidden_size, hidden_size)
        self.key = nn.Linear(hidden_size, hidden_size)
        self.value = nn.Linear(hidden_size, hidden_size)
        self.sentiment_weight = nn.Parameter(torch.tensor(1.0))
    ...
    def forward(self, inputs, input_ids):
        ...
        # 对情感词赋予额外的注意力
        sentiment_mask = torch.zeros_like(input_ids, dtype=torch.float).unsqueeze(-1).to(inputs.device)
        for i, ids in enumerate(input_ids):
            for j, id in enumerate(ids):
                token = tokenizer.convert_ids_to_tokens(id.item())
                if token in self.sentiment_words:
                    sentiment_mask[i, j] = self.sentiment_weight

        attention_scores += sentiment_mask
        attention_probs = nn.Softmax(dim=-1)(attention_scores)
        context = torch.matmul(attention_probs, value)
        return context

class RobertaWithCustomAttention(AutoModelForSequenceClassification):
    def __init__(self, config):
        super(RobertaWithCustomAttention, self).__init__(config)
        ...
        # 加载情感字典
        with open(f'../output/scientific_citation_sentiment_lexicon.json', 'r', encoding='utf-8') as f:
            self.sentiment_lexicon = json.load(f)

        self.sentiment_embeddings = nn.Embedding(2, config.hidden_size)  # 0: neutral, 1: sentiment word
        self.sentiment_attention = nn.MultiheadAttention(config.hidden_size, num_heads=1, batch_first=True)

    # def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
    #     outputs = self.bert(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)
    #     pooled_output = outputs.last_hidden_state[:, 0, :]  # Use the CLS token representation
    #     pooled_output = self.apply_sentiment_attention(pooled_output, input_ids)
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        ...
        # 创建情感mask
        sentiment_mask = torch.zeros_like(input_ids, dtype=torch.float)
        for i, sent in enumerate(input_ids):
            for j, token_id in enumerate(sent):
                token = self.bert.tokenizer.decode([token_id.item()])
                if token in self.sentiment_lexicon['positive'] or token in self.sentiment_lexicon['negative']:
                    sentiment_mask[i, j] = 1

        # 应用情感attention
        sentiment_embeddings = self.sentiment_embeddings(sentiment_mask.long())
        attn_output, _ = self.sentiment_attention(sequence_output, sentiment_embeddings, sentiment_embeddings)
    ...
    def apply_sentiment_attention(self, sequence_output, input_ids):
        ...
        for i, sent in enumerate(input_ids):
            for j, token_id in enumerate(sent):
                token = self.bert.tokenizer.convert_ids_to_tokens(token_id.item())
...
def main(args):
    ...
    model = RobertaWithCustomAttention.from_pretrained(model_dir, num_labels=num_labels, id2label=id2label, label2id=label2id).to(
        device)

    train_texts, train_labels, val_texts, val_labels, test_texts, test_labels = load_sentiment_datasets(
        test_size=test_size, seed=args.seed)
